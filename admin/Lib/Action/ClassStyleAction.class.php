 ...
 */// 文章
class ClassStyleAction extends CommonAction{
	public function index()
	{
		...
		{
			$cate_ids = D("ClassStyle")->getChildIds(intval($_REQUEST['cate_id']));
		...
		$cate_list = D("ClassStyle")-> where("status=1")-> findAll();
		//$cate_list = D("ClassStyle")-> toFormatTree($cate_list,$dispname_arr);
	...
	public function moveExam()
	{
		$name=$this->getActionName();
			...
			if (! empty ( $model )) {
				...
				$cate_info = D("ClassStyle")->getById($cate_id);
				if (isset ( $id )) {
					$condition = array ($pk => array ('in', explode ( ',', $id ) ) );
				$test_condition = $condition;
				$test_condition['type'] = 1;
