 ...
 */// 文章
class ClassTradeAction extends CommonAction{
	public function index()
	{
		...
		{
			$cate_ids = D("ClassTrade")->getChildIds(intval($_REQUEST['cate_id']));
		...
		$cate_list = D("ClassTrade")-> where("status=1")-> findAll();
		//$cate_list = D("ClassTrade")-> toFormatTree($cate_list,$dispname_arr);
	...
	public function delAttachment()
	{
		...
		if($rs = D("Attachment")->where("id=".$attachment_id)->delete())
		{
			...
			D("AttachmentLink")->where("module='Exam' and rec_id=".$article_id." and attachment_id=".$attachment_id)->delete();
			echo $rs;
		}
		else
		{
	...
	public function foreverdelete() {
		...
		if (! empty ( $model )) {
			...
			foreach($ids as $idd)
			{
				$names .= M("Exam")->where("id=".$idd)->getField("name_".DEFAULT_LANG_ID).",";
			}
			if($names!='')
			{
			...
			if (isset ( $id )) {
				...
				if (false !== $model->where ( $condition )->delete ()) {
					...
					//开始删除相关的附件
					$attachment_ids = D("AttachmentLink")->where("module='Exam' and rec_id=".$id)->findAll();
					D("AttachmentLink")->where("module='Exam' and rec_id=".$id)->delete();
					if($attachment_ids)
					{
					...
					//开始删除相关的留言
					$msgList = D("Message")->where(array ("rec_id" => array ('in', explode ( ',', $id ) ),'rec_module'=>'Exam' ))->findAll();
					D("Message")->where(array ("rec_id" => array ('in', explode ( ',', $id ) ),'rec_module'=>'Exam' ))->delete();
	...
	public function deletePreview()
	{
		...
		$a_info = D("Exam")->getById($article_id);
		if($a_info['preview']!="")
		{
			@unlink($this->getRealPath().$a_info['preview']);
			D("Exam")->where("id=".$article_id)->setField("preview","");
		}
		$this->success (L('DEL_SUCCESS'));
	}
	...
	public function moveExam()
	{
		$name=$this->getActionName();
			...
			if (! empty ( $model )) {
				...
				$cate_info = D("ClassTrade")->getById($cate_id);
				if (isset ( $id )) {
					$condition = array ($pk => array ('in', explode ( ',', $id ) ) );
				$test_condition = $condition;
				$test_condition['type'] = 1;
