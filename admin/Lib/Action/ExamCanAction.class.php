 ...
 */// 文章
class ExamCanAction extends CommonAction{
	public function index()
	{
		...
		{
			$cate_ids = D("ExamCate")->getChildIds(intval($_REQUEST['cate_id']));
		...
		$cate_list = D("ExamCate")-> where("status=1")-> findAll();
		$cate_list = D("ExamCate")-> toFormatTree($cate_list,$dispname_arr);
	...
	public function moveExam()
	{
		$name=$this->getActionName();
			...
			if (! empty ( $model )) {
				...
				$cate_info = D("ExamCate")->getById($cate_id);
				if (isset ( $id )) {
					$condition = array ($pk => array ('in', explode ( ',', $id ) ) );
				$test_condition = $condition;
				$test_condition['type'] = 1;
