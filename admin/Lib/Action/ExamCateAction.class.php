 ...
 */// 文章分类
class ExamCateAction extends CommonAction{
	...
	public function update()
	{
		...
		if (false !== $list) {
			...
			$child_cate_ids = D("ExamCate")->getChildIds($id);
			$child_cate_ids[] = $id;

			D("ExamCate")->where(array('id'=>array('in',$child_cate_ids)))->setField("type",$type);
			D("Exam")->where(array('cate_id'=>array('in',$child_cate_ids)))->setField("type",$type);
	...
	public function foreverdelete() {
		...
		if (! empty ( $model )) {
			...
			foreach($ids as $idd)
			{
				$names .= M("ExamCate")->where("id=".$idd)->getField("name_".DEFAULT_LANG_ID).",";
			}
			if($names!='')
			{
	...
	public function loadCateType()
	{
		...
		echo D("ExamCate")->where("id=".$cate_id)->getField("type");
	}
}
?>