...
class MajorAction extends CommonAction{
	public function index()
	{
		...
		{
			$cate_ids = D("School")->getChildIds(intval($_REQUEST['cate_id']));
		...
		$cate_list = D("School")-> where("status=1")-> findAll();
		$cate_list = D("School")-> toFormatTree($cate_list,$dispname_arr);
	...
	public function deletePreview()
	{
		...
		$a_info = D("Major")->getById($article_id);
		if($a_info['preview']!="")
		{
			@unlink($this->getRealPath().$a_info['preview']);
			D("Major")->where("id=".$article_id)->setField("preview","");
		}
		$this->success (L('DEL_SUCCESS'));
	}
	...
	public function moveArticle()
	{
		$name=$this->getActionName();
			...
			if (! empty ( $model )) {
				...
				$cate_info = D("School")->getById($cate_id);
				if (isset ( $id )) {
					$condition = array ($pk => array ('in', explode ( ',', $id ) ) );
				$test_condition = $condition;
				$test_condition['type'] = 1;
