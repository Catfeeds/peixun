 ...
 */
class PassportService extends Think
{
	...
	public function getLocalUser($identifier, $password = null)
	{
		...
		$identifier_type = is_numeric($identifier) ? 'mobile' : 'email';


		$user = D('User')->where($identifier_type=$identifier)->find();
		if (!$user)
			return false;
	...
	public function registerLogin(array $user, $is_remeber_me = false,$password = null)
	{
		...
		$this->_recordLogin($user['id'],$user['user_name'], $is_remeber_me);
		return $result;
	}
	...
	private function _recordLogin($uid,$uname, $is_remeber_me = false){
		...
		if (!$this->getCookieUid()) {
			$expire = $is_remeber_me ? (3600*24*365) : (1800*1);
